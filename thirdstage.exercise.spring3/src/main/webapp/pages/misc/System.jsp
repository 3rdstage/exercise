...
<title>Information on JVM and System</title>
...
<META name="Version" content="2.1, 2011-09-06, Enhance display format for values of classpath like system properties or env. variables"/>
<META name="Version" content="2.0.1, 2011-01-15, Add item for thread name and id"/>
...
<META name="Required-Java-Ver" content="1.6"/>
</head>
<body><a name='top'></a>
...
%>
	<div id="platform-mbeanserver" style='margin-left:40px;padding-top:20px;padding-bottom:20px'>
	<form name='jmxControl'>
		...
		<%
			...
			<tr>
				<td></td>
				<td>RMI Port</td>
				<td>: <input type='text' name="rmiPort" size='6' maxlength='6' value='9999'></input></td>
			</tr>
			<tr>
				<td></td>
		...
		<%
			...
			<tr>
				<td colspan='3' style='font-size:50%'>&nbsp;</td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td style='white-space:nowrap'>Thread processed this request. name(id)</td>
				<td>: <%=Thread.currentThread().getName()%>(<%=Thread.currentThread().getId()%>)</td>
			</tr>
...
<%
	...
	for(Object key : systemKeys){ %>
		<tr>
			...
			<%
			if("java.class.path".equals((String)key)
				|| "java.library.path".equals((String)key)
				|| "sun.boot.class.path".equals((String)key)
				|| "sun.boot.library.path".equals((String)key)){ %>
				<td><%= systemProps.getProperty((String)key) == null ? "" : systemProps.getProperty((String)key).replace(java.io.File.pathSeparator, java.io.File.pathSeparator + "<br>") %></td>
			<%	
			}else{ %>
	...
	for(String key : envKeys){ %>
		<tr>
			...
			if("path".equalsIgnoreCase(key)
				|| ((key != null) && key.toUpperCase().contains("CLASSPATH"))){ %>
				<td><%= env.get(key) == null ? "" : env.get(key).replace(java.io.File.pathSeparator, java.io.File.pathSeparator + "<br>") %></td>
			<%	
			}else{ %>
				<td><%=env.get(key) %></td>
			<%
