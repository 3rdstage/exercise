...
set DEPLOYED_HOME=%~dp0..
if exist "%DEPLOYED_HOME%\lib" (
   set CLASSPATH=%CLASSPATH%;%DEPLOYED_HOME%\lib\*
)
if exist "%DEPLOYED_HOME%\conf" (
   set CLASSPATH=%CLASSPATH%;%DEPLOYED_HOME%\conf
)

echo CLASSPATH = %CLASSPATH%

if not defined LOG_BASE (
   if exist "%DEPLOYED_HOME%\log" (
      set LOG_BASE=%DEPLOYED_HOME%\log
   ) else (
      set LOG_BASE=%TEMP%
   )
)
