...
<%!
//@TO-DO how to improve giving this page information to getLogger() method like ...getLogger(this.getPageName())
protected static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger("exercise/CatWithHibernate.jsp");
%>
...
<html>
<head>
<title>Simple Hibernate Test</title>
...
<meta name="Keywords" content="Hibernate, Hibernate Annotations"/>
...
<h1>Simple Hibernate Test ???</h1>
<br>
<h3>New Cat</h3>
<form name='registerForm' action='./SimpleHibernate.jsp' 
...
if(id != null && id.length() > 0){
	...
	try{
		sess = thirdstage.exercise.hibernate3.case1.HibernateUtil3.currentSession();
		tx = sess.beginTransaction();
		
		cat = new thirdstage.exercise.hibernate3.case1.Cat3();
		cat.setId(id.trim());
		cat.setName(name);
		...
		if(sess != null){
			try{ thirdstage.exercise.hibernate3.case1.HibernateUtil3.closeSession(); }
			catch(Exception ex){}
		}
	}
...
try{
	...
	org.hibernate.Query query = sess.createQuery("select c from Cat3 as c");
	for(java.util.Iterator it = query.iterate(); it.hasNext(); ){
		cat = (thirdstage.exercise.hibernate3.case1.Cat3)(it.next()); %>
	<tr>
		<td><%=cat.getId()%></td>
		<td><%=cat.getName()%></td>
		<td><%=cat.getSex()%></td>
